...
class Inventory(BaseProduct):
    ...
    def calculate_price_per_unit(self):
        ...
        if self.gold_type == 'Chinese Gold':
            price = gold_prices.get(f'chinese_{self.karat.lower()}', 0) * self.weight
        elif self.gold_type == 'Saudi Gold':
            price = gold_prices.get(f'saudi_{self.karat.lower()}', 0) * self.weight
        return price

    def __repr__(self):
        return f"<Inventory id={self.id}, name='{self.product_name}', quantity={self.quantity}>"
...
class SoldProduct(BaseProduct):
    ...
    def __repr__(self):
        return f"<SoldProduct id={self.id}, name='{self.product_name}', quantity_sold={self.quantity_sold}>"
